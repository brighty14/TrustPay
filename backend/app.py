...
FACE_MATCH_THRESHOLD = 0.75 if facenet_embedder is not None else 0.72
...
def decode_base64_image(face_image_base64):
    ...
    image_bytes = base64.b64decode(face_image_base64, validate=True)
...
def _opencv_face_embeddings(image):
    candidates = []
    for source_image in (image, enhance_low_light_image(image)):
        gray = cv2.cvtColor(source_image, cv2.COLOR_BGR2GRAY)
        gray = cv2.GaussianBlur(gray, (3, 3), 0)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
        gray = clahe.apply(gray)

        for candidate_gray in (gray, cv2.flip(gray, 1)):
            ...
            faces = frontal_detector.detectMultiScale(candidate_gray, scaleFactor=1.1, minNeighbors=5)
            if len(faces) != 1:
                faces = profile_detector.detectMultiScale(candidate_gray, scaleFactor=1.1, minNeighbors=5)
            ...
            if len(faces) == 1:
                ...
                end_x = min(x + w + padding, candidate_gray.shape[1])
                end_y = min(y + h + padding, candidate_gray.shape[0])
                candidates.append(candidate_gray[start_y:end_y, start_x:end_x])
            ...
            candidates.append(candidate_gray[start_y:start_y + crop_size, start_x:start_x + crop_size])

    embeddings = []
    seen = set()
    for face_crop in candidates:
        if face_crop.size == 0:
            continue
        ...
        key = tuple(np.round(embedding, 4))
        if key not in seen:
            seen.add(key)
...
def extract_face_embedding_candidates(face_image_base64):
    ...
    if facenet_embedder is not None:
        embeddings = []
        for candidate in (image, enhanced_image):
            rgb_image = cv2.cvtColor(candidate, cv2.COLOR_BGR2RGB)
            detections = facenet_embedder.extract(rgb_image, threshold=0.80)

            if len(detections) == 1:
                embeddings.append(detections[0]["embedding"].astype("float32").tolist())
        return embeddings

    return _opencv_face_embeddings(image)


def extract_face_embedding(face_image_base64):
    embeddings = extract_face_embedding_candidates(face_image_base64)
    if not embeddings:
        return None
    return embeddings[0]
...
def verify_face():
    ...
    try:
        live_embeddings = extract_face_embedding_candidates(face_image)
    except Exception:
        live_embeddings = []

    stored_embeddings = [embedding for embedding in stored_embeddings if embedding]

    if not stored_embeddings or not live_embeddings:
        return jsonify({
            "status": "error",
            "verified": False,
    ...
    similarity = round(float(max(similarity_scores)), 4)
